...
import { TStudentScheduleCalendarEvent, TStudentScheduleCalendarEventResponse } from "@/types/student.type";

interface ScheduleCalendarProps {
  calendarRef: React.RefObject<FullCalendar | null>;
  view: "day" | "week" | "month";
  events: TStudentScheduleCalendarEventResponse;
...
const ScheduleCalendar: React.FC<ScheduleCalendarProps> = ({
  ...
  // Transform events to FullCalendar EventInput format
  const fullCalendarEvents: any[] = useMemo(() => {
    let processedRawEvents = [...(events?.calendarEvents || [])];

    if (view === "month") {
      const groupedByDate: Record<string, TStudentScheduleCalendarEvent[]> = {};
    ...
    return processedRawEvents.map((event) => {
      const isFakeMore = (event.bookingStatus as any) === "MORE";
    ...
    <>
      ...
      <div className="flex-1 relative overflow-auto p-4 fc-custom-theme">
        ...
        <FullCalendar
          ...
          eventContent={(arg) => {
            const { tutorName, categoryName, startTimeLabel, endTimeLabel, isPast, isFakeMore } = arg.event.extendedProps;
            ...
            return (
              <div className={cn(
                "flex items-center gap-2 px-2 py-1 rounded-lg h-full w-full overflow-hidden shadow-sm transition-all border",
                isPast 
