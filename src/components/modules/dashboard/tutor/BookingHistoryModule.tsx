...
const BookingHistoryModule = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { className: string }> = {
      ...
      PENDING: {
        className:
          "bg-yellow-50 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400 border-yellow-100 dark:border-yellow-500/20",
      },
      CANCELLED: {
        className:
    ...
    return (
      <Badge
        variant="outline"
        className={`font-semibold px-3 py-1 uppercase tracking-wider text-[10px] ${style.className}`}
      >
        {status}
      </Badge>
    );
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Oversee your session requests from students.
        </p>
      </motion.div>
        ...
        <Card className="border shadow-sm overflow-hidden bg-card text-card-foreground">
          <CardContent className="p-0">
            ...
            <motion.div
              ...
              className="p-6 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 border-b border-border"
            >
              ...
              <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  ...
                  <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                    <Button
                      variant="default"
                      className="flex-1 sm:flex-none bg-primary hover:bg-primary/90 text-primary-foreground transition-colors gap-2 h-11 rounded-xl px-6"
                    >
              ...
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="w-[80px] font-bold text-muted-foreground uppercase text-xs tracking-wider text-center">
                      SL
                    </TableHead>
                    ...
                    <TableHead className="text-right font-bold text-muted-foreground uppercase text-xs tracking-wider pr-6">
                      ACTIONS
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={6}
                        className="h-60 text-center text-muted-foreground font-medium"
                      >
                        <motion.div
                    ...
                    <AnimatePresence mode="popLayout" initial={false}>
                      {data.data.map((booking, index) => (
                        <motion.tr
                          ...
                          exit="exit"
                          layout
                          className="hover:bg-muted/20 transition-colors border-b border-border group whitespace-nowrap"
                        >
                                ...
                                booking.meetingLink && (
                                  ...
                                  >
                                    <Button
                                      ...
                                      variant="ghost"
                                      disabled
                                      className="h-7 p-0 text-emerald-500 hover:text-emerald-400 hover:bg-transparent"
                                    >
            ...
            {!loading && data?.data && data.data.length > 0 && (
              <motion.div
                variants={itemVariants}
                className="p-6 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-border bg-muted/10"
              >
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        updateFilters({ limit: val, page: 1 })
                      }
                    >
                    ...
                    <PaginationContent className="gap-1 sm:gap-2">
                      <PaginationItem>
                        <PaginationPrevious
                          ...
                          className={`cursor-pointer h-9 px-3 rounded-lg border-border hover:bg-muted ${
                            currentPage === 1
      ...
      <motion.div
        variants={itemVariants}
        className="md:hidden flex items-center justify-center gap-2 text-[10px] text-muted-foreground font-bold uppercase tracking-widest py-2 bg-muted/20 rounded-lg"
      >
